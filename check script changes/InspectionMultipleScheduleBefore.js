...
for (thisElement in s_id1) {
	...
	var s_result = aa.inspection.getInspection(r.capId,r.inspId);
	if(s_result.getSuccess())
	{
		r.inspObj  = s_result.getOutput();	
	}
	else
	{
		logDebug("ERROR: Failed to retrieve inspection: " + s_result.getErrorMessage());
		r.inspObj = null;	
	}
...
for (thisResult in schedObjArray) {
    ...
    var inspType = null;
    var objClass = null;
    if (inspObj && inspObj != null)
    {
        var insp = inspObj.getInspection();
        if (insp && insp != null)
        {
            inspGroup = insp.getInspectionGroup();
        }
        inspType = inspObj.getInspectionType(); 
        objClass = inspObj.getClass();
    }
